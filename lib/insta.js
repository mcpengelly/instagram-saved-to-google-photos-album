...
var instagram_private_api_1 = __importDefault(require("instagram-private-api"));
// You must generate device id's before login. Id's generated based on seed
// So if you pass the same value as first argument - the same id's are generated every time
var ig = new instagram_private_api_1.default.IgApiClient();
...
var parseSavedPosts = function(
  ...
  var isVideoPost = lodash_1.default.has(savedPost, "video_codec");
  var isImagePost =
    lodash_1.default.has(savedPost, "image_versions2.candidates") &&
    !isVideoPost;
  var isCarouselPost =
    lodash_1.default.has(savedPost, "carousel_media") && !isVideoPost;
  if (isImagePost) {
    return lodash_1.default.last(
      lodash_1.default.get(savedPost, "image_versions2.candidates")
    ).url;
  } else if (isCarouselPost) {
    if (includeCarouselPosts) {
      return lodash_1.default
        .get(savedPost, "carousel_media")
        .map(function(post) {
          return lodash_1.default.last(
            lodash_1.default.get(post, "image_versions2.candidates")
          ).url;
        });
    }
    return;
